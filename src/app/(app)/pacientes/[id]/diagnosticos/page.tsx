    ...
    <main className="min-h-full bg-neutral-50 p-5 sm:p-8">
      <section className="mx-auto max-w-4xl space-y-6">
        <header className="rounded-2xl border bg-white p-5 shadow-sm">
          ...
          <p className="mt-4 text-sm font-medium text-neutral-500">CLIDENT · Diagnósticos</p>
          ...
          <section className="rounded-2xl border bg-white p-5 shadow-sm">
            ...
            <form action={crearDiagnosticoDesdeFormulario} className="mt-4 space-y-4">
              ...
              <div className="flex justify-end">
                <button className="rounded-lg bg-neutral-900 px-4 py-2 text-sm font-medium text-white">Registrar diagnóstico</button>
              </div>
            </form>
          </section>
        ) : null}
        ...
        <section className="space-y-3">
          ...
          {diagnosticos.length === 0 ? (
            ...
            diagnosticos.map((diagnostico) => (
              <article
                key={diagnostico.id}
                className={`rounded-2xl border bg-white p-5 shadow-sm ${diagnostico.anulado ? "opacity-70" : ""}`}
              >
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {diagnostico.anulado ? (
                        ...
                        <span className="rounded-full bg-emerald-50 px-2.5 py-0.5 text-xs font-medium text-emerald-700">Vigente</span>
                      )}
                    </div>
                    {diagnostico.dientes.length > 0 ? (
                      ...
                      <p className="mt-2 text-xs text-neutral-500">General del paciente</p>
                    )}
                    {diagnostico.notas ? <p className="mt-2 whitespace-pre-wrap text-sm text-neutral-700">{diagnostico.notas}</p> : null}
