      ...
      <section className="mx-auto max-w-6xl space-y-6">
        <header className="rounded-2xl border bg-card p-5 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <Link href="/pacientes" className="text-sm text-muted-foreground underline-offset-4 hover:underline">← Pacientes</Link>
              <p className="mt-4 text-sm font-medium text-muted-foreground">CLIDENT · Expediente</p>
              <h1 className="mt-1 text-2xl font-semibold tracking-tight">{paciente.nombres} {paciente.apellidos}</h1>
              <p className="mt-1 text-sm text-muted-foreground">Nacimiento: {fechaLarga(paciente.fechaNacimiento)} · Teléfono: {paciente.telefono}</p>
            </div>
            {tienePermiso(ctx.roles, "agenda:write") ? (
              <Link href={`/agenda/nueva?pacienteId=${encodeURIComponent(paciente.id)}`} className="rounded-lg bg-primary transition-colors hover:bg-rosa-hover px-4 py-2 text-center text-sm font-medium text-primary-foreground">
                + Agendar cita
              </Link>
            ) : null}
          </div>
        ...
        <section id="resumen" className="grid gap-6 lg:grid-cols-2">
          <article className="rounded-2xl border bg-card p-5 shadow-sm">
            ...
            {!puedeVerPii ? <p className="mt-5 rounded-lg bg-muted px-3 py-2 text-xs text-muted-foreground">El DUI se muestra enmascarado según tu rol.</p> : null}
          </article>
        ...
        {puedeLeerClinico ? (
          <section id="alertas" className="rounded-2xl border border-advertencia/30 bg-advertencia-suave p-5 shadow-sm">
            <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <p className="text-sm font-medium text-foreground">Información clínica</p>
                <h2 className="mt-1 text-lg font-semibold text-foreground">Alertas médicas activas</h2>
                <p className="mt-1 text-sm text-foreground">Solo el personal clínico puede ver o modificar esta sección.</p>
              </div>
            </header>

            {estadoAlerta === "expediente-no-disponible" || estadoAlerta === "no-disponible" ? (
              <p role="alert" className="mt-4 rounded-lg border border-advertencia/40 bg-card px-3 py-2 text-sm text-foreground">
                La alerta ya no está disponible. Recargá la ficha antes de volver a intentarlo.
              </p>
            ) : null}

            {alertas.length === 0 ? <p className="mt-5 text-sm text-foreground">No hay alertas médicas activas registradas.</p> : (
              <ul className="mt-5 space-y-3" aria-label="Alertas médicas activas">
                {alertas.map((alerta) => (
                  <li key={alerta.id} className="rounded-xl border border-advertencia/30 bg-card p-4" role="alert">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <h3 className="font-semibold text-foreground">{alerta.titulo}</h3>
                        {alerta.detalle ? <p className="mt-1 whitespace-pre-wrap text-sm text-foreground">{alerta.detalle}</p> : null}
                        <p className="mt-2 text-xs text-muted-foreground">Registrada por {alerta.creadaPorNombre}.</p>
                      </div>
                      {puedeEscribirClinico ? (
                        <details className="text-sm">
                          <summary className="cursor-pointer font-medium text-foreground">Desactivar</summary>
                          <form action={desactivarAlertaMedicaDesdeFormulario} className="mt-3 w-full space-y-2 sm:w-72">
                            ...
                            <label className="block text-xs font-medium text-foreground">Motivo de desactivación
                            ...
                            <button className="rounded-lg border border-destructive/40 px-3 py-1.5 text-xs font-medium text-destructive">Confirmar desactivación</button>
                          </form>
                        </details>
                      ) : null}
                    </div>
            ...
            {puedeEscribirClinico ? (
              <form action={crearAlertaMedicaDesdeFormulario} className="mt-5 grid gap-3 border-t border-advertencia/30 pt-5 sm:grid-cols-[minmax(0,1fr)_auto]">
                <input type="hidden" name="pacienteId" value={paciente.id} />
                <div className="grid gap-3">
                  <label className="block text-sm font-medium text-foreground">Nueva alerta médica
                    <input name="titulo" required maxLength={160} placeholder="Ej.: Alergia a penicilina" className="mt-1 w-full rounded-lg border bg-card px-3 py-2 font-normal" />
                  </label>
                  <label className="block text-sm font-medium text-foreground">Detalle opcional
                    <textarea name="detalle" maxLength={1000} rows={2} placeholder="Contexto que el personal clínico debe conocer." className="mt-1 w-full rounded-lg border bg-card px-3 py-2 font-normal" />
                  </label>
                </div>
                <div className="self-end">
                  <button className="rounded-lg bg-primary transition-colors hover:bg-rosa-hover px-4 py-2 text-sm font-medium text-primary-foreground">Registrar alerta</button>
                </div>
              </form>
            ) : null}
          </section>
        ...
        <section id="agenda" className="overflow-hidden rounded-2xl border bg-card shadow-sm">
          <header className="flex items-center justify-between gap-4 border-b p-5">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">Últimas 20 citas de esta clínica.</p>
            </div>
          ...
          {citas.length === 0 ? (
            <p className="p-8 text-center text-sm text-muted-foreground">Todavía no hay citas registradas para este paciente.</p>
          ) : (
                ...
                <tbody className="divide-y">
                  {citas.map((cita) => (
                    <tr key={cita.id} className={cita.estado === "CANCELADA" ? "bg-muted text-muted-foreground" : ""}>
                      ...
                      <td className="px-5 py-4"><span className="rounded-full bg-muted px-2.5 py-1 text-xs font-medium">{cita.estado}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
