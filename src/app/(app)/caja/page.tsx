    ...
    <main className="min-h-full bg-neutral-50 p-5 sm:p-8">
      <section className="mx-auto max-w-5xl space-y-6">
        <header className="rounded-2xl border bg-white p-5 shadow-sm">
          <p className="text-sm font-medium text-neutral-500">CLIDENT · Caja</p>
        ...
        {pacientes.length > 0 ? (
          <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
            <h2 className="border-b bg-neutral-50 px-5 py-3 text-sm font-semibold uppercase tracking-wide text-neutral-600">Pacientes</h2>
            <ul className="divide-y text-sm">
              {pacientes.map((paciente) => (
                <li key={paciente.id}>
                  <Link href={`/caja/${paciente.id}`} className="flex items-center justify-between px-5 py-3 hover:bg-neutral-50">
                    <span className="font-medium">{paciente.apellidos}, {paciente.nombres}</span>
                    <span className="text-neutral-500">Estado de cuenta →</span>
                  </Link>
                </li>
              ))}
            </ul>
        ...
        <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
          <header className="border-b bg-neutral-50 px-5 py-3">
          ...
          {pendientes.length === 0 ? (
            <p className="p-8 text-center text-sm text-neutral-600">No hay procedimientos pendientes de cobro.</p>
          ) : (
                  ...
                  {pendientes.map((procedimiento) => (
                    <tr key={procedimiento.id}>
                      ...
                      <td className="px-5 py-3 text-right">
                        <Link href={`/caja/${procedimiento.pacienteId}`} className="font-medium text-neutral-900 underline-offset-4 hover:underline">Cobrar</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
