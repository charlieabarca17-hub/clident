...
export default async function setup(): Promise<void> {
  ...
  try {
    await migrator.query(
      "TRUNCATE TABLE citas, estados_superficie, eventos_odontograma, diagnostico_dientes, diagnosticos, desactivaciones_alertas_medicas, alertas_medicas, expedientes, pacientes, tratamientos, categorias_tratamiento, auditoria, membresias, sucursales, usuarios, clinicas RESTART IDENTITY CASCADE",
    );
  } finally {
    await migrator.end();
  }
